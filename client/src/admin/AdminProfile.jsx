...
export default function AdminProfile() {
  ...
  let newErrors = {};
    ...
    } else if (!/^[A-Za-z ]+$/.test(admin.UserName)) {
    ...
    } else if (!/^[0-9]{10}$/.test(admin.UserCNo)) {
  ...
  useEffect(() => {
    const fetchAdmin = async () => {
      try {
        const res = await axios.get(`${API_BASE}/api/admin/${adminEmail}`);
        setAdmin(res.data);
        if (res.data.Photo) {
          setPreview(`${API_BASE}/uploads/${res.data.Photo}`);
        } else {
          setPreview("");
        }
        setLoading(false);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(`${API_BASE}/api/admin/update/${adminEmail}`, formData, {
      ...
      <div className="admin-card">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <input
            ...
            required
          />
          {errors.UserCNo && <p style={{ color: "red", fontSize: "0.8rem" }}>{errors.UserCNo}</p>}

          <label>Email ID</label>
          <input
            type="email"
            name="UserEmailID"
            value={admin.UserEmailID}
            readOnly
