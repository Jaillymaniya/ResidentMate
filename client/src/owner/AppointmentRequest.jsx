...
export default function AppointmentRequests() {
  ...
  //     return;
  //   }
  //   fetch(`http://localhost:5000/api/appointments/owner?ownerId=${ownerId}`)
  //     .then((res) => {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:5000/api/appointments/getOwnerByEmail?email=${email}`)
      .then((res) => {
      ...
      .then((owner) => {
        ...
        return fetch(`http://localhost:5000/api/appointments/owner?ownerId=${owner.ownerId}`);
  ...
  const handleCheckout = (id) => {
    if (window.confirm("Mark this appointment as checkout?")) {
      fetch(`http://localhost:5000/api/appointments/${id}/checkout`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" }
      })
        .then(res => res.json())
